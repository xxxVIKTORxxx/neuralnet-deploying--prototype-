...
def NN_move():
    ...
    elif Agent.get_state()[2]:
        move_down()

    elif Agent.get_state()[3]:
        move_up()  
...
layer_in = [n for n in range(0,11)]
...
def draw_chart(size):
    ...
    if chart == True:
        ...
        df_plot = df.plot(kind="line", grid=True, ax=ax, title='Life Length', xlabel='Score', color = [col/255 for col in AGENT_COL])
